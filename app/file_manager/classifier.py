...
def classify_files(dfFiles: pd.DataFrame, list_classification: dict, classification_type: str = "name", 
    ...
    def process_row(idx):
        ...
        try:
            if classification_type == "name":
                file_classification = classify_file_with_name(filename=filename, list_classification=list_classification)
    ...
    try:
        if(save_grist):
            update_records_in_grist(dfFilesClassified, 
                              key_column='filename', 
        ...
        if(save_path != None):
            dfFilesClassified.to_csv(f'{save_path}/dfFichiersClassifiés_{directory_path.split("/")[-1]}_{getDate()}.csv', index = False)
            print(f"Liste des fichiers sauvegardées dans {save_path}/dfFichiersClassifiés_{directory_path.split("/")[-1]}_{getDate()}.csv")
    except Exception as e:
...
DIC_CLASS_FILE_BY_NAME = {
    ...
    "lettre_consultation": {
        "words": [["lettre", "consultation"], ["inivation", "soumissionner"], [" dce "], ["consultation"], ["offre", "financière"], ["lc"], ["lconsultation"]],
        "stopwords": [],
    ...
    "bon_de_commande": {
        "words": [["bon", "de", "commande"], [" bdc "], [" bc "], ["EXE2"], [" bc1"], [" bc2"], [" bc3"], [" bc4"], [" bc5"], [" bc6"], [" bc7"], [" bc8"], [" bc9"], [" bc10"]],
        "stopwords": [],
