...
def select_attachments_for_classification(offset=0, limit=200):
    return select_attachments(
        columns=["filename", "extension", "dossier", "num_EJ", "date_creation", "taille", "hash", "text", "is_ocr", "nb_mot"],
...
def select_engagements_with_attachments(columns_ej=None, columns_attachments=None, list_cat=None, offset=0, limit=200):
    ...
    if columns_ej is None:
        columns_ej = ["num_EJ", "Designation", "Descriptif_prestations", "Date", "Prestataire", "SIRET", "Administration", "Sources_et_conflits"]
    if columns_attachments is None:
        columns_attachments = ["filename", "extension", "dossier", "date_creation", "taille", "hash", "text", "is_ocr", "nb_mot", "llm_response", "json_error", "classification", "classification_type"]
    order_by = ['engagements.num_EJ', 'filename', 'extension', 'hash']
    if list_cat is None:
        req_synthesis = "1=1"  # Always true condition
    else:
        req_synthesis = "attachments.llm_response is not null and attachments.classification in (" + f"'{"','".join(list_cat)}'" + ")"
    columns_with_prefix = [f"engagements.{column}" for column in columns_ej] + [f"attachments.{column}" for column in columns_attachments]
    SQL = f"""
