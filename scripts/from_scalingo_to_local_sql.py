...
def load_local_sql_engagements(dfFiles, connexion):
    ...
    data = [
            tuple(record)
            for record in dfToSend.to_records(index=False)
        ]
...
def load_local_sql_attachments(dfFiles, connexion):
    columns = ["filename", "extension", "dossier", "num_EJ", "date_creation", "taille", "hash", "text", "is_ocr", "nb_mot",
               "relevant_content", "is_embedded", "llm_response", "json_error", "classification", "classification_type"]
...
def remove_engagements_local_sql(dfFiles, connexion):
    ...
    data = [
            tuple(record)
            for record in dfToRemove.to_records(index=False)
        ]
