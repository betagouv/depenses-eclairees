...
def download_documents(start: datetime=None):
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
        future_to_doc = {
            executor.submit(_task_download, doc.external_id, doc.name): doc
        for doc in qs}
        for i, future in enumerate(as_completed(future_to_doc)):
            ...
            try:
                ...
                logger.exception("[%s/%s] Error downloading document %s %s: %s", i, len(future_to_doc), doc.external_id, doc.name, exc)
            else:
